...
class InventoryView(ttk.Toplevel):
    ...
    def populate_products_list(self):
        ...
        for row in products:
            ...
            # tree columns: ID, Código, Categoría, Nombre, Precio, Stock, Valorizado, U. Medida, Tipo Op., Emisor, Dirección
            price = row[2]
            stock = row[3]
    ...
    def add_product(self):
        ...
        if code and not database.is_code_unique(code, issuer_name, issuer_address):
            messagebox.showerror("Error de Validación", f"El código '{code}' ya existe para esta empresa/dirección. Por favor use un código único.", parent=self)
            return
    ...
    def update_product(self):
        ...
        if code != current_code and code and not database.is_code_unique(code, current_issuer, current_address):
            messagebox.showerror("Error de Validación", f"El código '{code}' ya existe para esta empresa/dirección. Por favor use un código único.", parent=self)
            return
    ...
    def generate_unique_code(self):
        ...
        issuer_name = self.filter_issuer_var.get()
        issuer_address = self.filter_address_var.get()
        if issuer_name == "Todas" or issuer_address == "Todas":
            issuer_name = None
            issuer_address = None
        while True:
            code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=length))
            if database.is_code_unique(code, issuer_name, issuer_address):
